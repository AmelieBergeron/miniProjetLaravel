...
class EnsureUserIsAdmin
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($utilisateur !== null) {
            ...
            // Si on se trouve dans l'API et qu'on essaie de faire une action bloquée par le middleware sans être admin
            if ($request->bearerToken() && $request->accepts('application/json')) {
              //  dd("allo");
                return response(array("Erreur"=>"Veuillez vous authentifier avec un compte administrateur"));
